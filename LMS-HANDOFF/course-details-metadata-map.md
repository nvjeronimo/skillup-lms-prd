...
`Incomplete` (number on the brand tint), `Locked` (muted number — the lock stays on the right of the row, where the *Unlocks* tooltip anchors), with a `Number` text property. It replaces
